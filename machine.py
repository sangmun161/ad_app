...
MODEL_DEFECT_PATH = r"defect_classifier.pkl"
...
DEFECT_KNOWLEDGE_BASE = {
    'Center': {
        'korean': '센터 불량',
        'cause': 'CBCMP',
        'action': '이제/센터 구간 CMP 편차 여부 확인'
    },
    'Donut': {
        'korean': '도넛형 불량',
        'cause': 'CBCMP',
        'action': '패드 상태, 압력 조건, 슬러리 공급 균일성 점검'
    },
    'Edge-Loc': {
        'korean': '엣지 국부 불량',
        'cause': 'PC, RMG',
        'action': 'PC 공정 전·후 표면 클리닝 상태 점검, 설비 상태(온도, 압력, 회전/이송 조건 등) 변동 이력 확인'
    },
    'Edge-Ring': {
        'korean': '엣지 링 불량',
        'cause': 'RMG',
        'action': '웨이퍼 중심/에지 구간별 결함 분포 비교'
    },
    'Loc': {
        'korean': '국부 불량',
        'cause': 'PC',
        'action': 'PC 공정 전·후 표면 클리닝 상태 점검'
    },
    'Near-full': {
        'korean': '전면 불량',
        'cause': '심각한 장비 고장, 원자재 불량',
        'action': '즉시 생산 중단 및 장비 전수 점검'
    },
    'Random': {
        'korean': '랜덤 불량',
        'cause': '정전기(ESD), 미세 스크래치',
        'action': 'ESD 방지 대책 및 이송 환경 점검'
    },
    'Scratch': {
        'korean': '스크래치',
        'cause': '물리적 접촉, 슬러리 이물질',
        'action': '패드 상태, 압력 조건, 슬러리 공급 균일성 점검'
    }
}
...
DEFECT_DOMAIN_KB = {
    9: {
        "title": "CBCMP, PC, RMG – 9번 유형 (가성 불량 False)",
        "features": (
            '<span style="color:red">검출면적</span>, <span style="color:red">에너지값</span>'
        ),
        "cause": (
            "• 검출면적이 클수록 강한 가성일 확률 높음<br>"
    ...
    10: {
        "title": "RMG – 10번 유형(미세 파티클 Small Particle)",
        "features": (
            '<span style="color:red">기준편차</span>, 명도수준'
        ),
        "cause": (
            "• 광학/센서 계측 불안정으로 인한 밝기 기반 결함<br>"
            "• 기준편차 증가로 센서 드리프트 가능성<br>"
            "• 실제 결함보다 장비 조건 영향 가능"
        ),
        "action": (
            "• 조명·포커스·센서 캘리브레이션 점검<br>"
            "• 재검을 통한 계측/실결함 구분<br>"
            "• 기준편차·노이즈 트렌드 확인"
        )
    },
    ...
    14: {
        "title": "PC – 14번 유형 (버블 Bubble)",
        "features": (
            '<span style="color:red">명도수준</span>, 영역잡음, 잡음정도'
        ),
        "cause": (
            "• 국부 영역 잡음 집중, 파티클/잔사/오염 가능<br>"
    ...
    17: {
        "title": "PC – 17번 유형(포토레지스트 잔여물 PR Residue)",
        "features": (
            '<span style="color:red">명도수준</span>, 잡음정도'
        ),
        "cause": (
            "• 물리적 손상성 결함, 방향성 라인/스크래치 가능<br>"
            "• 특정 방향 편중 패턴 발생 가능<br>"
            "• 직전/PC 공정의 기계적 접촉 영향"
        ),
        "action": (
            "• 롤러/가이드 등 접촉 부위 점검<br>"
            "• 방향성 결함 패턴 확인<br>"
            "• 장비 내부 이물 점검"
        )
    },
    ...
    20: {
        "title": "RMG – 20번 유형(거대 파티클 Large Particle)",
        "features": (
            '<span style="color:red">기준편차</span>, 명도수준'
        ),
        "cause": (
    ...
    21: {
        "title": "CBCMP, RMG – 21번 유형 (금속 잔여물 Metal Residue)",
        "features": (
    ...
    22: {
        "title": "PC – 22번 유형(마이크로 스크래치 Micro-Scratch)",
        "features": (
            '<span style="color:red">명도수준</span>, 에너지값'
        ),
        "cause": (
            "• 강한 국부 충돌/파손 이벤트 가능<br>"
    ...
    28: {
        "title": "PC – 28번 유형 (패턴 불량 Pattern Bridge)",
        "features": (
            '<span style="color:red">명도수준</span>, 정형지수'
        ),
        "cause": (
            "• 형상 뚜렷한 패턴 결함<br>"
            "• 방향성 긴 스크래치 가능<br>"
    ...
    56: {
        "title": "CBCMP – 56번 유형 (패드 자국 Pad Mark)",
        "features": (
            '<span style="color:red">검출면적</span>, '
    ...
    99: {
        "title": "CBCMP – 99번 유형 (미분류 Unclassified)",
        "features": (
            '<span style="color:red">검출면적</span>, '
            '<span style="color:red">정형지수</span>, 명도수준, 기준편차'
        ),
        "cause": (
            "• 면적·형상·명암·변동성 복합 결함<br>"
            "• 공정 변동성 증가 신호<br>"
            "• 여러 요인 누적 가능"
        ),
        "action": (
            "• CBCMP 전후 조건 이력 점검<br>"
            "• 수율/명도/신호 트렌드 확인<br>"
            "• 복합 유형으로 원인 세분화"
        )
    }
...
def load_defect_model():
    ...
    try:
        with open(MODEL_DEFECT_PATH, "rb") as f:
            obj = pickle.load(f)
        ...
        if isinstance(obj, dict):
            ...
            for v in obj.values():
                if hasattr(v, "predict"):
                    return v, None
            return None, "❌ defect_classifier.pkl 내부에서 predict 가능한 모델을 찾지 못했습니다."
        return obj, None
    except Exception as e:
...
def compute_false_direction(input_df, df_final, model_rf, feature_cols):
    """
    국소 민감도 기반 방향성 분석:
    - 피처를 약간 올리거나/내렸을 때 가성(FALSE) 확률이 증가하는 방향 탐색
    """
    directions = {}
    try:
...
def show_page(df_final: pd.DataFrame):
    """
    main.py에서 df_final을 넘겨주며 이 함수가 실행됨.
    """
